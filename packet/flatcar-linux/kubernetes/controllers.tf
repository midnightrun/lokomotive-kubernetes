resource "null_resource" "dns-etcd" {
  provisioner "local-exec" {
    command = format(
    "echo '\n\n-= Create the following DNS %s-> %s: and then  `touch /tmp/dns1`; I WILL WAIT HERE =-\n\n'; while ! test -f /tmp/dns1; do sleep 1; done",
    format("%s-etcd%d.%s.", var.cluster_name, 0, var.dns_zone),
    packet_device.controllers[0].access_private_ipv4,
    )
  }
}

resource "null_resource" "dns-apiservers" {
  provisioner "local-exec" {
    command = format(
    "echo '\n\n-= Create the following DNS %s-> %s: and then  `touch /tmp/dns2`; I WILL WAIT HERE =-\n\n'; while ! test -f /tmp/dns2; do sleep 1; done",
    format("%s.%s.", var.cluster_name, var.dns_zone),
    join(",", packet_device.controllers.*.access_public_ipv4),
    )
  }
}

resource "null_resource" "dns-apiservers_private" {
  provisioner "local-exec" {
    command = format(
    "echo '\n\n-= Create the following DNS %s-> %s: and then  `touch /tmp/dns3`; I WILL WAIT HERE =-\n\n'; while ! test -f /tmp/dns3; do sleep 1; done",
    format("%s-private.%s.", var.cluster_name, var.dns_zone),
    join(",", packet_device.controllers.*.access_private_ipv4),
    )
  }
}
